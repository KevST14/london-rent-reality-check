...
| [`03_price_model`](notebooks/03_price_model.ipynb) | Build, check, tune, explain the model, and add a rough band. | Held-out test: about **£60 mean error, 22% median error, R-squared 0.72**. Beats a linear baseline by about £8. SHAP explains every prediction; a quantile-model band covers ~75% of held-out prices. |
| [`03b_model_deep_dives`](notebooks/03b_model_deep_dives.ipynb) | What shapes did the model learn, which listings does it disagree with, and why is Brent hard? | PDP/ICE for the top features (station distance goes flat once centrality is known), a plausibility-filtered mispricing scan, and an honest, inconclusive dig into Brent. |
| [`03c_model_comparison`](notebooks/03c_model_comparison.ipynb) | Does the gradient-boosting library choice matter here? | No. LightGBM and scikit-learn's HistGradientBoosting tie; XGBoost defaults are slightly behind. Kept scikit-learn because it needs no system library for the deploy. |
